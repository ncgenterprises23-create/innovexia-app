...
import { getImportFMSConfig, updateImportFMSConfig } from '@/lib/sheets';
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const result = await updateImportFMSConfig(config);
        return NextResponse.json({ success: true, message: 'Configuration updated', result });
    } catch (error) {
        console.error('Error saving Import FMS config:', error);
