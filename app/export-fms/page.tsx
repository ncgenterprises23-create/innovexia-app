...
const emptyForm = { id: '', piNumber: '', partyName: '', containerType: '', products: [] as string[] };

const EXPORT_STAGES = [
    { step: 1, name: 'Check Advance Remittance' },
    { step: 2, name: 'Credit the Remittance' },
    { step: 3, name: 'Fill Export Check list' },
    { step: 4, name: 'Share Export Checklist with Pankaj' },
    { step: 5, name: 'Stock Done' },
    { step: 6, name: 'TRUCK BOOKING' },
    { step: 7, name: 'Check Freight & Demand Container' },
    { step: 8, name: 'Loading Done' },
    { step: 9, name: 'ACCOUNT PROCESS' },
    { step: 10, name: 'Docs Sharing' },
    { step: 11, name: 'Send Documents to Client' },
    { step: 12, name: 'Check Remittance of Balance Payment' },
    { step: 13, name: 'Telex or Courier BL' },
    { step: 14, name: 'Credit the Balance Remittance' },
    { step: 15, name: 'ENTER FOREIGN EXCAHNGE DIFFERENCE' },
    { step: 16, name: 'Complete BRC' },
    { step: 17, name: 'Make a Folder & Upload Docs' },
];

const STEP_CHECKLISTS: Record<number, string[]> = {
    9: ['E-WAYBILL', 'BILLING', 'CHECK BUSSY STOCK AFTER BILLING', 'E-INVOICE'],
    10: ['Loading Docs Checklist', 'Documents to Truck Driver - Invoice', 'Documents to Truck Driver - Packing list', 'Documents to Truck Driver - Eway Bill', 'Documents to Truck Driver - Tax Invoice', 'Documents Mail to Agent - Invoice', 'Documents Mail to Agent - Packing list', 'Documents Mail to Agent - Eway Bill', 'Documents Mail to Agent - Tax Invoice'],
    11: ['PROFORMA INVOICE', 'COMMERCIAL INVOICE', 'PACKING LIST', 'CERTIFICATES', 'DATE LIST', 'CONTAINER BOOKING DOCUMENT', 'LOADING PICTURES', 'EAN CODE LIST OF APPLICABLE', 'BL DRAFT', 'FINAL BL SCAN'],
    14: ['Check the total amount of advance remittance with the invoice total.'],
    17: ['Invoice (In PDF and excel)', 'Packing List (in PDF and Excel)', 'BL', 'Shipping Bill', 'Freight Agent All Bills Scan Copy', 'Deposit Remittance copy', 'Balance Remittance Copy', 'BRC document', 'Drawback Receipt bank Statement', 'IGST Refund Receipt bank Statement'],
};
...
export default function ExportFMSPage() {
    const [data, setData] = useState<ExportFMS[]>([]);
    ...
    const [editingItem, setEditingItem] = useState<ExportFMS | null>(null);
    const [deletingItem, setDeletingItem] = useState<ExportFMS | null>(null);
    const [cancellingItem, setCancellingItem] = useState<ExportFMS | null>(null);
    ...
    const fetchConfig = async () => {
        try {
            const res = await fetch('/api/export-fms-config');
            if (res.ok) {
    ...
    const activeData = useMemo(() => {
        ...
        if (viewMode === 'data' && activeTimeFilter) {
            ...
            filtered = filtered.filter(item => {
                ...
                if (currentStep > 17) return false;
    ...
    const getCurrentStep = (item: ExportFMS) => {
        let step = 1;
        for (let s = 1; s <= 17; s++) {
    ...
    const handleBulkUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const updatePromises = Array.from(selectedItems).map(id => {
                ...
                if (itemsToMarkDone.has(item.id) && currentStep <= 17) {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mt-0.5">
                                EXPORT ORDER TO DELIVERY TRACKING
                            </p>
                        </div>
                    </div>
                                ...
                                <div className="flex gap-2 min-w-max pr-2 pb-4">
                                    {(() => {
                                        ...
                                        EXPORT_STAGES.forEach((s, idx) => {
                                            statTiles.push({ step: s.step, label: s.name, value: statusStats[`Step${s.step}`] });
                                        });
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-xs">
                                        {loading ? (
                                            <tr><td colSpan={21} className="px-6 py-12 text-center text-slate-400 font-bold">Loading records...</td></tr>
                                        ) : paginatedData.length === 0 ? (
                                            <tr><td colSpan={21} className="px-6 py-12 text-center text-slate-400 font-bold">No records found</td></tr>
                                        ) : paginatedData.map((item, idx) => {
                                            const products = parseProducts(item['Product']);
                                            return (
                                                <tr key={item.id || `row-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 group transition-colors">
                                                    ...
                                                    <td className="px-4 py-4">
                                                        <div className="flex items-center justify-center gap-1.5">
                                                            ...
                                                            <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }}
                                                                onClick={() => openRemoveModal(item.id, item['PI Number'])}
                                                    ...
                                                    <td className="px-6 py-4 border-r border-slate-100 dark:border-slate-700">
                                                        <div className="flex flex-col gap-0.5">
                                                            <span className="font-bold text-gray-900 dark:text-white text-sm leading-tight">{item['PI Number']}</span>
                                                            {item['Party Name'] && (
                                                            ...
                                                            {item['Container Type'] && (
                                                                <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                                                                    Container: {item['Container Type']}
                                                                </span>
                                                            )}
                                                            {products.length > 0 && (
                                                                <div className="flex flex-wrap gap-1 mt-1">
                                                                    {products.map((p: string, i: number) => (
                                                                        <span key={i} className="text-[9px] bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 px-2 py-1 rounded">
                                                                            {p}
                                                                        </span>
                                                                    ))}
                                                                </div>
                        ...
                        <Fragment key="modal-export-fms">
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl border border-gray-100 dark:border-gray-800 overflow-hidden text-gray-900 dark:text-gray-100">
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-2">PI Number <span className="text-red-500">*</span></label>
                                        <input type="text" value={form.piNumber} onChange={(e) => setForm({ ...form, piNumber: e.target.value })} className="w-full px-3 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[var(--theme-primary)] outline-none text-sm transition-all" placeholder="Enter PI Number" />
                                    </div>

                                    <div>
                                        <label className="block text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-2">Party Name</label>
                                        <input type="text" value={form.partyName} onChange={(e) => setForm({ ...form, partyName: e.target.value })} className="w-full px-3 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[var(--theme-primary)] outline-none text-sm transition-all" placeholder="Enter Party Name" />
                                    </div>

                                    <div>
                                        <label className="block text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-2">Container Type</label>
                                        <select value={form.containerType} onChange={(e) => setForm({ ...form, containerType: e.target.value })} className="w-full px-3 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[var(--theme-primary)] outline-none text-sm transition-all">
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-2">Products</label>
                                        <div className="space-y-2">
                                            <div className="flex gap-2">
                                                <input type="text" value={productInput} onChange={(e) => setProductInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addProduct())} className="flex-1 px-3 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[var(--theme-primary)] outline-none text-sm transition-all" placeholder="Enter product name" />
                                                <button type="button" onClick={addProduct} className="bg-[var(--theme-primary)] hover:bg-[var(--theme-primary)]/90 text-gray-900 px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all"><Plus className="w-5 h-5" /></button>
                                            </div>
                                            {form.products.length > 0 && (
                                                <div className="flex flex-wrap gap-2 p-2 bg-gray-50 dark:bg-gray-800/30 rounded-xl">
                                                    {form.products.map((product: string, idx: number) => (
                                                        <div key={idx} className="bg-[var(--theme-primary)]/10 text-[var(--theme-primary)] px-3 py-1.5 rounded-full flex items-center gap-2 text-[10px] font-bold">
                                                            <span>{product}</span>
                                                            <button type="button" onClick={() => removeProduct(idx)} className="hover:opacity-70 transition-opacity"><X className="w-3.5 h-3.5" /></button>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 dark:border-gray-800 overflow-hidden text-gray-900 dark:text-gray-100">
                                    ...
                                    <div className="p-6 space-y-4">
                                        <p className="text-gray-700 dark:text-gray-300 text-sm">
                                            Are you sure you want to delete PI Number <strong>{deletingItem?.['PI Number']}</strong>? This action cannot be undone.
                                        </p>
                        ...
                        <Fragment key="modal-cancel">
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 dark:border-gray-800 overflow-hidden text-gray-900 dark:text-gray-100">
                                    ...
                                    <div className="p-6 space-y-4">
                                        <p className="text-gray-700 dark:text-gray-300 text-sm">
                                            Are you sure you want to {cancellingItem?.['Cancelled']?.toLowerCase() === 'yes' ? 'restore' : 'cancel'} PI Number <strong>{cancellingItem?.['PI Number']}</strong>?
                                        </p>
                                        <div className="flex gap-2 justify-end">
                                                                ...
                                                                <div className="flex flex-col gap-3">
                                                                    <div className="flex items-center justify-between gap-3">
                                                                        <div className="min-w-0">
                                                                            <h4 className="text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-tight leading-tight truncate">{item['PI Number']}</h4>
